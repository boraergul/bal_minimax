...
| Search | PostgreSQL Full-Text (全文检索) | Arama işlevselliği |
...
| GET | `/fifo-tavsiye/{urun_id}` | FIFO önerisi |
| POST | `/transfer` | Depo transferi |
| GET | `/uyari` | Minimum stok uyarıları |
| GET | `/son-kullanma` | Son kullanma yaklaşanlar |
...
| GET | `/lot/{lot_no}` | Lot detayı (izlenebilirlik) |
...
| Yöntem | Endpoint | Açıklama |
|--------|----------|----------|
| GET | `/stok/anlik` | Anlık stok durumu |
| GET | `/stok/deger` | Stok değeri raporu |
| GET | `/satis/ozet` | Satış özeti |
| GET | `/satis/musteri/{id}` | Müşteri satış geçmişi |
| GET | `/satis/urun/{id}` | Ürün satış geçmişi |
| GET | `/tedarikci/performans/{id}` | Tedarikçi performansı |
| GET | `/uretim/ozet` | Üretim özeti |
| GET | `/uretim/fire-analiz` | Fire oranı analizi |
| GET | `/izlenebilirlik/lot/{lot_no}` | Lot izlenebilirlik raporu |
...
erDiagram
    ...
    KULLANICILERE ||--o{ TEDARIKCI_DEGERLENDIRMELERI : "oluşturur"
    TEDARIKCILER ||--o{ TEDARIKCI_URUNLER : "tedarik eder"
    TEDARIKCILER ||--o{ STOK_KARTLARI : "sağlar"
    TEDARIKCILER ||--o{ TEDARIKCI_DEGERLENDIRMELERI : "değerlendirilir"
    URUNLER ||--o{ TEDARIKCI_URUNLER : "tedarik edilir"
    URUNLER ||--o{ STOK_KARTLARI : "bulunur"
    URUNLER ||--o{ URUN_OZELLIKLERI : "tanımlanır"
    MUSTERILER ||--o{ SATIS_KAYITLARI : "yapar"
    STOK_KARTLARI ||--o{ STOK_HAREKETLERI : "hareket eder"
    STOK_KARTLARI ||--o{ LOT_IZLENEBILIRLIK : "kaynak olur"
    URETIM_EMIRLERI ||--o{ URETIM_DETAYLARI : "içerir"
    URETIM_DETAYLARI ||--o{ STOK_KARTLARI : "tüketir"
    SATIS_KAYITLARI ||--o{ SATIS_KALEMLERI : "içerir"
    SATIS_KALEMLERI ||--o{ STOK_KARTLARI : "referans"
```
...
|-------|----------|------------------|
| `stok_kartlari` | Tüm lot/stok kayıtları | Büyük (milyonlarca satır) |
| `stok_hareketleri` | Stok hareket logları | Çok büyük (milyarlarca satır potansiyeli) |
| `lot_izlenebilirlik` | İzlenebilirlik zinciri | Orta |
| `satış_kalemleri` | Satış detayları | Büyük |

### 5.3 İndeks Stratejisi

```sql
-- Stok kartları: lot_no ile hızlı arama
CREATE INDEX idx_stok_lot_no ON stok_kartlari(lot_no);
CREATE INDEX idx_stok_urun_id ON stok_kartlari(urun_id);
CREATE INDEX idx_stok_tedarikci_id ON stok_kartlari(tedarikci_id);
CREATE INDEX idx_stok_son_kullanma ON stok_kartlari(son_kullanma) WHERE aktif = true;

-- Stok hareketleri: tarih aralığı sorguları
CREATE INDEX idx_hareket_stok_id ON stok_hareketleri(stok_id);
CREATE INDEX idx_hareket_tarih ON stok_hareketleri(created_at);
CREATE INDEX idx_hareket_tip ON stok_hareketleri(hareket_tipi);

-- Satış: müşteri ve tarih bazlı
CREATE INDEX idx_satis_musteri ON satis_kayitlari(musteri_id);
CREATE INDEX idx_satis_tarih ON satis_kayitlari(tarih);
...
ALTER TABLE stok_kartlari ADD CONSTRAINT chk_son_kullanma
  CHECK (son_kullanma > uretim_tarihi);

-- Miktar sıfırdan küçük olamaz
ALTER TABLE stok_kartlari ADD CONSTRAINT chk_miktar_pozitif
  CHECK (miktar >= 0);

-- Fiyat sıfırdan küçük olamaz
ALTER TABLE stok_kartlari ADD CONSTRAINT chk_fiyat_pozitif
  CHECK (birim_fiyat >= 0);

-- Foreign key cascade
-- Tedarikçi silindiğinde stok kartları soft delete'e uğrar
-- Ürün silindiğinde stok kartları soft delete'e uğrar
```
...
flowchart TB
    ...
    subgraph L2["Düzey 2: Veritabanı Ölçekleme"]
        ...
        Partition[("Table Partitioning<br/>stok_hareketleri")]
    end
...
| Table partitioning | `stok_hareketleri` (aylık) | > 10M satır |
